...
<head>
    ...
    <style>
        ...
        .tradition-tab.locked { background: #f8f9fa; color: #6c757d; cursor: not-allowed; opacity: 0.6; }
        .tradition-tab.locked:hover { background: #f8f9fa; }
...
<body>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Please enter your email to start mapping ragas. Your contributions will be attributed to you.</p>
                        <form id="loginForm">
                            <div class="mb-3">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary" onclick="login()">Start Mapping</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-body py-2">
                    <div class="row text-center">
                        <div class="col-md-3">
                            <h6 class="text-muted mb-1">Carnatic Progress</h6>
                            <div class="progress mb-1" style="height: 8px;">
                                <div class="progress-bar" id="carnatic-progress" role="progressbar" style="width: 0%"></div>
                            </div>
                            <small id="carnatic-stats" class="text-muted">0/0 mapped</small>
                        </div>
                        <div class="col-md-3">
                            <h6 class="text-muted mb-1">Hindustani Progress</h6>
                            <div class="progress mb-1" style="height: 8px;">
                                <div class="progress-bar" id="hindustani-progress" role="progressbar" style="width: 0%"></div>
                            </div>
                            <small id="hindustani-stats" class="text-muted">0/0 mapped</small>
                        </div>
                        <div class="col-md-3">
                            <h6 class="text-muted mb-1">Cross-tradition</h6>
                            <div class="progress mb-1" style="height: 8px;">
                                <div class="progress-bar bg-secondary" id="cross-progress" role="progressbar" style="width: 0%"></div>
                            </div>
                            <small id="cross-stats" class="text-muted">Locked</small>
                        </div>
                        <div class="col-md-3">
                            <h6 class="text-muted mb-1">User</h6>
                            <small id="user-info" class="text-muted">Not logged in</small>
                            <br>
                            <button class="btn btn-sm btn-outline-secondary mt-1" onclick="logout()" style="display: none;" id="logout-btn">Logout</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="panel left-panel">
                ...
                <div class="panel-content">
                    ...
                    <div class="tradition-tabs">
                        <div class="tradition-tab active" data-tradition="carnatic" onclick="switchTradition('carnatic')">
                            Carnatic (<span id="carnatic-count">{{ mapper.saraga_ragas.carnatic|length }}</span>)
                        </div>
                        <div class="tradition-tab" data-tradition="hindustani" onclick="switchTradition('hindustani')">
                            Hindustani (<span id="hindustani-count">{{ mapper.saraga_ragas.hindustani|length }}</span>)
                        </div>
                        <div class="tradition-tab locked" data-tradition="cross" onclick="showCrossTraditionLocked()">
                            Cross-tradition (Locked)
                        </div>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const savedUser = localStorage.getItem('ragaMapperUser');
            if (savedUser) {
        ...
        function login() {
            ...
            localStorage.setItem('ragaMapperUser', JSON.stringify(currentUser));
            updateUserInfo();
        ...
        function logout() {
            currentUser = null;
            localStorage.removeItem('ragaMapperUser');
            updateUserInfo();
        ...
        function showCrossTraditionLocked() {
            alert('Cross-tradition mapping is locked. Complete Carnatic and Hindustani mappings first to unlock this feature.');
        }
        
        function switchTradition(tradition) {
            if (tradition === 'cross') {
                showCrossTraditionLocked();
                return;
            }
        ...
        function confirmMapping(saragaRaga, ramanarunachalamRaga, confidence, matchType) {
            if (!currentUser) {
                alert('Please login first to confirm mappings');
                return;
            }
        ...
        function updateTraditionProgress() {
            // Update Carnatic progress
            fetch('/api/ragas?tradition=carnatic&filter=all')
                .then(response => response.json())
                .then(carnaticRagas => {
                    fetch('/api/ragas?tradition=carnatic&filter=mapped')
                        .then(response => response.json())
                        .then(mappedCarnatic => {
                            const total = carnaticRagas.length;
                            const mapped = mappedCarnatic.length;
                            const percentage = total > 0 ? (mapped / total) * 100 : 0;
                            
                            document.getElementById('carnatic-progress').style.width = percentage + '%';
                            document.getElementById('carnatic-stats').textContent = `${mapped}/${total} mapped`;
                        });
                });
            
            // Update Hindustani progress
            fetch('/api/ragas?tradition=hindustani&filter=all')
                .then(response => response.json())
                .then(hindustaniRagas => {
                    fetch('/api/ragas?tradition=hindustani&filter=mapped')
                        .then(response => response.json())
                        .then(mappedHindustani => {
                            const total = hindustaniRagas.length;
                            const mapped = mappedHindustani.length;
                            const percentage = total > 0 ? (mapped / total) * 100 : 0;
                            
                            document.getElementById('hindustani-progress').style.width = percentage + '%';
                            document.getElementById('hindustani-stats').textContent = `${mapped}/${total} mapped`;
                            
                            // Check if cross-tradition should be unlocked
                            checkCrossTraditionUnlock();
                        });
                });
        }
        
        function checkCrossTraditionUnlock() {
            // Unlock cross-tradition if both Carnatic and Hindustani are 80% complete
            const carnaticProgress = document.getElementById('carnatic-progress').style.width;
            const hindustaniProgress = document.getElementById('hindustani-progress').style.width;
            
            const carnaticPercent = parseFloat(carnaticProgress) || 0;
            const hindustaniPercent = parseFloat(hindustaniProgress) || 0;
            
            if (carnaticPercent >= 80 && hindustaniPercent >= 80) {
                const crossTab = document.querySelector('[data-tradition="cross"]');
                crossTab.classList.remove('locked');
                crossTab.onclick = () => switchTradition('cross');
                crossTab.textContent = 'Cross-tradition';
                document.getElementById('cross-stats').textContent = 'Unlocked';
                document.getElementById('cross-progress').classList.remove('bg-secondary');
            }
        }
...
def api_confirm_mapping():
    ...
    # Add user information to the mapping
    notes = f"Mapped by {data.get('user_name', 'Unknown')} ({data.get('user_email', 'unknown@example.com')}) on {data.get('mapped_at', datetime.now().isoformat())}"
    ...
    # Save user mapping to JSON file for tracking
    save_user_mapping(data)
    
    return jsonify({'success': True, 'mapping': mapping})

def save_user_mapping(mapping_data):
    """Save user mapping to JSON file for tracking"""
    import json
    from datetime import datetime
    
    user_mapping = {
        'saraga_raga': mapping_data['saraga_raga'],
        'ramanarunachalam_raga': mapping_data['ramanarunachalam_raga'],
        'confidence': mapping_data['confidence'],
        'match_type': mapping_data['match_type'],
        'user_email': mapping_data.get('user_email'),
        'user_name': mapping_data.get('user_name'),
        'mapped_at': mapping_data.get('mapped_at', datetime.now().isoformat())
    }
    ...
    # Add new mapping
    user_mappings.append(user_mapping)
